...
class PrintInformationPatches(QObject):
    ...
    def _formatdAffixes(self) -> None:
        ...
        replacements = {
            ...
            "{line_width}": self._abbreviate_number(extruder_stack.getProperty("line_width", "value")),           
            ...
            "{material_length}": str(round(self._print_information.materialLengths[extruder_nr]) if extruder_nr < len(self._print_information.materialLengths) else 0),
            ...
            "{date_iso}": QDate.currentDate().toString(format=Qt.ISODate).replace(":", "-"),
            "{date_year}": QDate.currentDate().toString("yy"),
            "{date_month}": QDate.currentDate().toString("MM"),
            "{date_day}": QDate.currentDate().toString("dd"),
            "{time_iso}": QTime.currentTime().toString(format=Qt.ISODate).replace(":", "-"),
